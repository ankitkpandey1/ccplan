...
impl Scheduler for NativeScheduler {
    ...
    fn add(&self, trigger: &TriggerRecord) -> Result<(), SchedulerError> {
        ...
        let label = label(&trigger.backend_id);
...
fn launchd_plist_xml(path: &Path, label: &str, binary: &Path, trigger: &TriggerRecord) -> String {
    ...
    let arguments = arguments
        ...
        .collect::<Vec<_>>()
        .join("\n");
    let interval = start_calendar_interval(trigger.scheduled_at);
    format!(
...
fn start_calendar_interval(timestamp: Timestamp) -> CalendarInterval {
...
fn label(backend_id: &str) -> String {
